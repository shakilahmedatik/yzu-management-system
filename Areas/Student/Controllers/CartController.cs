...
namespace YZU.Areas.Student.Controllers
{
    [Area("Student")]
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var cart = _db.CourseCart.Where(c => c.ApplicationUserId == claim.Value);
            if (cart != null)
            {
        ...
        public async Task<IActionResult> SummaryPost(string stripeToken)
        {
            ...
            detailCart.listCart = await _db.CourseCart.Where(c => c.ApplicationUserId == claim.Value).ToListAsync();
            ...
            _db.CourseCart.RemoveRange(detailCart.listCart);
            HttpContext.Session.SetInt32(SD.ssCourseCartCount, 0);
            await _db.SaveChangesAsync();
        ...
        {
            var cart = await _db.CourseCart.FirstOrDefaultAsync(c => c.Id == cartId);
            cart.Count += 1;
            await _db.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
        ...
        public async Task<IActionResult> Minus(int cartId)
        {
            ...
            if (cart.Count == 1)
            {
                _db.CourseCart.Remove(cart);
                await _db.SaveChangesAsync();

                var cnt = _db.CourseCart.Where(u => u.ApplicationUserId == cart.ApplicationUserId).ToList().Count;
